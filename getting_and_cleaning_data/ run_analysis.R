...
## Add directory and filename (txt or csv) of the clean/tidy data:
        tidyDataFile <- "./tidy-UCI-HAR-dataset.txt"
## tidyDataFileAVG <- "./tidy-UCI-HAR-dataset-AVG.csv"
## Add directory and filename (.txt) of the clean/tidy data
...
# Combine the data table, train vs. test, by the rows...
...
# Add friendly names to features column
        names(features) <- c('feat_id', 'feat_name')
# Search for the matches to argument mean or standard deviation within each element of character vector
...
## 4) Appropriately labels the data set with descriptive activity names...
        ...
        # Combines the data table by the columns
                tidyDataSet <- cbind(s, y, x)

        
# 5) Creates a 2nd independent tidy data set w/ the average of each variable for each activity and each subject...
        p <- tidyDataSet[, 3:dim(tidyDataSet)[2]] 
        tidyDataAVGSet <- aggregate(p,list(tidyDataSet$Subject, tidyDataSet$Activity), mean)
        
        # Put Activity and Subject name of the columns 
